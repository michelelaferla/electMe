...
export function ElectionsPage({ onSelect }: Props) {
  ...
  return <main className="page"><header className="topbar"><div><h1>Choose election</h1><p>{profile?.full_name}</p></div><button onClick={() => supabase.auth.signOut()}>Sign out</button></header>
    ...
    {!elections.length && !error && <p>No valid elections are currently available.</p>}
  </main>;
}
